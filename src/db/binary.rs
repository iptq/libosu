...
mod tests {
    ...
    fn test_read_uleb128() {
        ...
        assert_eq!(num.read_uleb128().unwrap(), 624485);
    }
    ...
    fn test_write_uleb128() {
        ...
        curs.write_uleb128(624485).unwrap();
    ...
    fn test_read_uleb128_string() {
        ...
        assert_eq!(reader.read_uleb128_string().unwrap(), text.to_string());

        let mut reader_empty = Cursor::new(vec![0x00]);
        assert_eq!(reader_empty.read_uleb128_string().unwrap(), String::new());

        let mut reader_0_len = Cursor::new(vec![0x0B, 0x00]);
        assert_eq!(reader_0_len.read_uleb128_string().unwrap(), String::new());
    }

    #[test]
    fn test_write_uleb128_string() {
        let mut buf = Vec::new();
        let mut curs = Cursor::new(&mut buf);
        curs.write_uleb128_string("Hello, world!").unwrap();
